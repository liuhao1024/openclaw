...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - Dashboard shortcuts keep common chat actions keyboard-accessible: `/` focuses the composer, `N` jumps to new messages when the affordance is visible, `Esc` dismisses transient dashboard UI, and `Cmd/Ctrl+K` opens the command palette. These shortcuts do not fire from editable controls or embedded editor roots.
